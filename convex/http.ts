...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    return new Response("Convex HTTP routes are working!", {
      status: 200,
...
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || "AIzaSyCLWmQD7fjXwtlzAMuPUUcGLzR8tnn2tNA");
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    try {
      ...
      if (!fitness_level) missingFields.push("fitness_level");
      ...
      console.log("User details:", { age, height, weight, fitness_goal, fitness_level });
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    try {
      console.log("=== VAPI Test Webhook Received ===");
      const payload = await request.json();
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "VAPI webhook is working correctly",
